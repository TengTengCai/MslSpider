...
class MslspiderPipeline(object):
    ...
    def add_content(self, item):
        sql = f'INSERT INTO religion4(title, tag, categories, question, answer, qa_id, url_mark, r_type, lang) ' \
              f'values (\'{escape_string(item["title"])}\', \'{escape_string(item["tag"])}\', ' \
              f'\'{escape_string(item["categories"])}\', \'{escape_string(item["question"])}\', ' \
              f'\'{escape_string(item["answer"])}\', \'{escape_string(item["qa_id"])}\', ' \
              f'\'{escape_string(item["url_mark"])}\', \'{escape_string(item["r_type"])}\', ' \
              f'\'{escape_string(item["lang"])}\')'
        print(sql)
        try:
            self.db.ping(reconnect=True)
            with self.db.cursor() as cursor:
